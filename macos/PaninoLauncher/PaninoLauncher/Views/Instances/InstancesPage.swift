...
private struct InstanceImmersiveBackground: View {
    let instance: GameInstance?
...
private struct InstanceImmersivePrimary: View {
    let instance: GameInstance?
    let totalCount: Int
...
private struct InstanceImmersiveControls: View {
    let instance: GameInstance?
...
private struct InstanceImmersiveShelf: View {
    @Binding var searchText: String
    @Binding var sort: InstanceSort
    @Binding var filter: InstanceFilter
    let counts: [InstanceFilter: Int]
...
struct InstancesPage: View {
    ...
    @ViewBuilder
    private var selectedInstanceInspector: some View {
        if let selected = instanceStore.selectedInstance {
            GlassPanel(showsShadow: false, surfaceLevel: .panel) {
                VStack(alignment: .leading, spacing: 12) {
                    InstanceCardCover(instance: selected)
                        .frame(height: 132)
                        .clipShape(RoundedRectangle(cornerRadius: PaninoTokens.Radius.card, style: .continuous))
                    ...
                    VStack(alignment: .leading, spacing: 5) {
                        ...
                        MetadataLine(items: [
                            ...
                            selected.loaderTitle(language: theme.language)
                        ], font: .caption.weight(.medium))
                    }

                    LazyVGrid(columns: [GridItem(.adaptive(minimum: 116), spacing: 8)], alignment: .leading, spacing: 8) {
                        LaunchMetric(
                            title: localizedString(theme.language, english: "Memory", chinese: "内存", italian: "Memoria", french: "Mémoire", spanish: "Memoria"),
                            value: "\(selected.memoryMb) MB"
                        )
                        LaunchMetric(
                            title: localizedString(theme.language, english: "Status", chinese: "状态", italian: "Stato", french: "État", spanish: "Estado"),
                            value: selected.status.title(language: theme.language)
                        )
                    }

                    VStack(alignment: .leading, spacing: 8) {
                        GlassButton(systemImage: "play.fill", title: AppText.launch.localized(theme.language), prominent: true) {
                            launchSelectedInstance(selected)
                        }
                        .disabled(!viewModel.canSubmitTask || !GameConfigurationCapabilities.capabilities(for: selected).canLaunch)
                        ...
                        GlassButton(systemImage: "folder", title: AppText.openFolder.localized(theme.language)) {
                            FinderIntegration.openInstanceDirectory(selected)
                        }
                    }
                }
            }
