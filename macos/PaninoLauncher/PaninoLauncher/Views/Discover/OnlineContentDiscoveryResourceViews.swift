...
extension OnlineContentDiscoveryPage {
    var discoverSectionBar: some View {
        GlassPanel {
            LazyVGrid(columns: catalogColumns, alignment: .leading, spacing: 8) {
    ...
    func discoverCatalogButton(_ section: DiscoverSection) -> some View {
        ...
        .background {
            RoundedRectangle(cornerRadius: PaninoTokens.Radius.control, style: .continuous)
                .fill(selectedSection == section ? theme.semanticSelectionColor : Color(nsColor: .controlBackgroundColor).opacity(0.28))
        }
    }
        ...
        ViewThatFits(in: .horizontal) {
            HStack(alignment: .top, spacing: theme.fontDensity.spacing) {
                ...
                if selectedProject != nil {
                    selectedProjectDetailContent(showBackButton: false)
    ...
    @ViewBuilder
    func selectedProjectDetailContent(showBackButton: Bool) -> some View {
    ...
    func categoryFilterButton(title: String, isSelected: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            ...
            .frame(height: 28)
            .background(Color(nsColor: .controlBackgroundColor).opacity(isSelected ? 0.42 : 0.24), in: RoundedRectangle(cornerRadius: 7, style: .continuous))
            .overlay {
                RoundedRectangle(cornerRadius: 7, style: .continuous)
                    .stroke(isSelected ? theme.semanticSelectionColor.opacity(0.65) : Color(nsColor: .separatorColor).opacity(0.28), lineWidth: 1)
            }
